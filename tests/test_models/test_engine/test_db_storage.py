...
class TestDBStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_count(self):
        """Test method for obtainig an instance db storage"""
        storage = models.storage
        storage.reload()
        state_data = {"name": "Kiambuu"}
        state_instance = State(**state_data)
        storage.new(state_instance)

        city_data = {"name": "Roysambu", "stae_id": state_instance.id}
        ...
        all_occurence = storage.count(State)
